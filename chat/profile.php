...
$res1 = mysql_query("SELECT * FROM `userdata` WHERE id='".$row1['id']."' AND TIMESTAMPDIFF(MINUTE, last_active_timestamp, NOW()) > 1;");
if($res1 === FALSE) {
                                ...
                                <div class="chat_options" style="background-color: #eceff1;" >
                                    <div class="drupalchat-self-profile">
                                        <div class="drupalchat-self-profile-div">
                                            <?php
                                            $queryses = "SELECT * FROM userdata where id = '".$_SESSION['id']."'";
                                            $resultses = mysql_query($queryses);
                                            $rowses = mysql_fetch_assoc($resultses);
                                            ?>
                                            <div class="drupalchat-self-profile-img + localhost-avatar-sprite-28 <?php echo strtoupper($_SESSION['username'][0]); ?>_3">
                                                <?php if(!empty($rowses['picname'])) {?>
                                                    <img src="storage/user_image/small<?php echo $rowses['picname']; ?>"/>
                                                <?php } ?>
                                            </div>
                                        </div>
                                        <div class="drupalchat-self-profile-namdiv">
                                            <a class="drupalchat-profile-un drupalchat_cng"><?php echo $_SESSION['username']; ?></a>
                                        </div>
